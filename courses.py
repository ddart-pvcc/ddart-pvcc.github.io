...
def process_network_courses():
        global network_elect #this set must be global since it is changed in the function
        global num_net_req, num_net_elect, tot_net, all_net_courses
        temp_set = set() #create temporary set empty set

        for course in network_elect:
                asterisk_course = "*" + course # insert asterisk in front of ELECTIVE course name
                temp_set.add(asterisk_course) # then add the new course to the temporary set
...
def display_network_courses():
        global out
        out.write("\nCERTIFICATE: Computer & Network Technologies")
        out.write(dash_line)
...
def process_info_courses():
        global info_elect #this set must be global since it is changed in the function
